...
We begin our explanation by assuming the existance of a \emph{reference domain model} which can be used as a ground truth to assess the learned domain model. 
Under this assumption, we define several evaluation metrics that measure the similirity of the learned domain model to the reference domain model,
the predictive power of the learned domain, and its usefulness in solving problems in the environment. Most of these metrics have been proposed in prior work~\cite{aineto2019learning}, and we discuss the pros and cons of using each metric. 


% \roni{TODO: Examples: grounded vs. lifted, different action names, different object types, paramter ordering, etc.}
Then, we discuss the limitation of depending on a reference domain model, and the challenge of comparing action models that use different state and action representations. This challenge is a very practical one, since ddomain model learning algorithms vary in the representation of the domain models they output. For example, some algorithms output models in a grounded representation~\cite{stern2017efficient,others}, while others output models in a lifted representation~\cite{juba2021safe,xi2024neuro,aineto2019learning}. Some example represent states based only the parameters of executed actions~\cite{locm}, others require a richer symbolic representation of states~\cite{juba2021safe}, while others use a visual representation of states~\cite{xi2024neuro}.
...
In this setting, we assume that there exists a \textit{reference domain model}, to be used as a ground truth to assess the learned domain model. Specifically, we can consider three main dimensions to optimize the learning process and to evaluate the learned model:
%
...
is that these objectives are in general not necessarily aligned. 
...
Next, we discuss possible and existing metrics for evaluating the three dimensions discussed in the previous section. 
...
Prior work often focused on domain model metrics that aim for the first objective (similarity), which are sometimes referred to as \emph{syntactic similarly} metrics~\citep{aineto2019learning,mordoch2023safe,xi2024neuro}. In literature, two models that are syntactically identical have been defined as \textit{strongly equivalent} \cite{chrpa2023comparing}

\roni{Others who have used, please add citations}
These metrics typically compare the intersection or difference of the predicates in the actions' preconditions and effects between the learned and reference domain models. We define these common metrics below. 
